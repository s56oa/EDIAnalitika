...
- **Detekcija duplikatov** — samodejno zazna dvojne zveze; privzeti način `po znaku`; opcija `po znaku + načinu`; gumba v glavi tabele "Vse zveze" preklopita med načinoma; duplikati so označeni z značko DUPE; grafikoni in metrike temeljijo le na veljavnih zvezah
- **Izvoz CSV** — gumb "Izvozi CSV" prenese celotno QSO tabelo kot `.csv` datoteko z UTF-8 BOM; semicolon-ločena; stolpci: `#`, `Date`, `Time (UTC)`, `Callsign`, `Mode`, `Locator`, `Distance (km)`, `Azimuth (°)`, `Duplicate`; vse zveze vključene (veljavne in duplikati)
...
Testi pokrivajo vse čiste funkcije logike (brez DOM): `parseEDI`, `locToLatLon`, `haversine`, `bearing`, `getCountry`, `escapeHTML`, `modeName`, `mapThemeColors`, `exportCSV`, `markDupes`, `formatLogTime`, `getLogHistory`, `saveLogToStorage`, `clearStoredLog`, razvrščanje QSO tabele — skupaj 201 testnih primerov v CLI.
...
- **Duplicate detection** — automatically detects duplicate QSOs; default mode `by callsign`; option `by callsign + mode`; toggle buttons in the "All QSOs" table header switch between modes; duplicates are marked with a DUPE badge; all charts and metrics are based on valid QSOs only
- **CSV export** — "Export CSV" button downloads the full QSO table as a `.csv` file with UTF-8 BOM; semicolon-delimited; columns: `#`, `Date`, `Time (UTC)`, `Callsign`, `Mode`, `Locator`, `Distance (km)`, `Azimuth (°)`, `Duplicate`; all QSOs included (valid and duplicates)
...
Tests cover all pure logic functions (no DOM): `parseEDI`, `locToLatLon`, `haversine`, `bearing`, `getCountry`, `escapeHTML`, `modeName`, `mapThemeColors`, `exportCSV`, `markDupes`, `formatLogTime`, `getLogHistory`, `saveLogToStorage`, `clearStoredLog`, QSO table sorting — 201 test cases via CLI.
